...
{
    Log(libecap::flXaction | libecap::ilDebug) << "applying filter to openxml file";
...
{
    if (zip_name_locate(archive, "[Content_Types].xml", ZIP_FL_NOCASE)  == -1)
        return false;

    if (zip_name_locate(archive, "docProps/core.xml", ZIP_FL_NOCASE) == -1)
        return false;
