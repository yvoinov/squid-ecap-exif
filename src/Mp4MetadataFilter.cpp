...
bool Mp4MetadataFilter::IsFtypSupported(const char* atom) const
{
    ...
    if (atom_type[0] == '3' && atom_type[1] == 'g' && atom_type[2] == 'p')
        return true;

    if (atom_type[0] == 'q' && atom_type[1] == 't')
        return true;

    if (atom_type[0] == 'i' && atom_type[1] == 's' && atom_type[2] == 'o')
        return true;

    if (atom_type[0] == 'M' && atom_type[1] == '4' && atom_type[2] == 'A')
        return true;
    ...
    uint64_t size)
{
    ...
    while (position < moov_atom_size)
    {
        ...
        if (atom_size + position > moov_atom_size || atom_size == 0)
            throw MetadataFilter::Exception("Failed to parse moov children");
